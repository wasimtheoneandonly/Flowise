...
class BaseCommand extends core_1.Command {
    ...
    async init() {
        ...
        if (flags.ALLOW_BUILTIN_DEP)
            process.env.ALLOW_BUILTIN_DEP = flags.ALLOW_BUILTIN_DEP;
        // Database config
        ...
        // Security
        if (flags.CUSTOM_MCP_SECURITY_CHECK)
            process.env.CUSTOM_MCP_SECURITY_CHECK = flags.CUSTOM_MCP_SECURITY_CHECK;
        if (flags.CUSTOM_MCP_PROTOCOL)
            process.env.CUSTOM_MCP_PROTOCOL = flags.CUSTOM_MCP_PROTOCOL;
        if (flags.HTTP_DENY_LIST)
            process.env.HTTP_DENY_LIST = flags.HTTP_DENY_LIST;
        if (flags.TRUST_PROXY)
            process.env.TRUST_PROXY = flags.TRUST_PROXY;
    }
}
